...
# FRONTEND_TYPE selects which frontend is deployed (see settings_main / .env.template):
#   "react"     → FastAPI serves the prebuilt SPA (forecastic/build); source = frontend_react/deploy
#   "streamlit" → the Streamlit app (unchanged default)
if FRONTEND_TYPE == "react":
    application_path = PROJECT_ROOT / "frontend_react" / "deploy"
else:
...
def get_app_files(
    ...
    # React deploy: the FastAPI entrypoint plus the prebuilt SPA it serves.
    if FRONTEND_TYPE == "react":
        source_files.append(
            (str(forecastic_path / "rest_api.py"), "forecastic/rest_api.py")
        )
        build_dir = forecastic_path / "build"
        source_files += [
            (str(f), str(f.relative_to(PROJECT_ROOT)))
