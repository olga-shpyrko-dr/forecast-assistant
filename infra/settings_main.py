...
# Selects which frontend Pulumi deploys: "react" or "streamlit".
# Default "streamlit" preserves the original behavior when FRONTEND_TYPE is unset.
FRONTEND_TYPE = os.environ.get("FRONTEND_TYPE", "streamlit").strip().lower()
if FRONTEND_TYPE not in ("react", "streamlit"):
    raise ValueError(
        f"FRONTEND_TYPE must be 'react' or 'streamlit', got {FRONTEND_TYPE!r}."
    )
