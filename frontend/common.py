...
"""Shared state, helpers, and the global sidebar - used by both app.py (the entry
point) and every page under pages/, so navigating between pages doesn't lose the
active forecast/filters/settings."""
...
def eyebrow(text: str, margin_bottom: str = "2px") -> str:
    """The 'FORECAST ANALYSIS'-style eyebrow label markup, for reuse on other pages."""
    return (
        f"<p style='font-family:\"Fragment Mono\",monospace;font-size:0.7rem;"
        f"text-transform:uppercase;letter-spacing:0.1em;color:#81FBA5;"
        f"margin-bottom:{margin_bottom};'>{text}</p>"
    )


def series_narrative_key(display_series: str | None) -> str:
    return display_series if display_series is not None else ""


def build_series_narratives(
    ...
    for series in series_keys:
        key = series_narrative_key(series)
...
def render_sidebar() -> None:
    """Global sidebar: dataset/filter form + display/commentary settings. Persists
    every result into st.session_state so both pages can read it regardless of
    which page triggered the run."""
    ...
    with st.sidebar:
        with st.form(key="sidebar_form"):
            ...
            st.selectbox(
                ...
                help=gettext(
                    "Select the dataset version to replay. Defaults to the latest."
                ),
            )
            ...
            sidebar_submit = st.form_submit_button(label=gettext("Run Forecast"))
    ...
    with st.spinner(gettext("Processing forecast...")):
        ...
        selected_version = next(
            (
                v
                for v in st.session_state["dataset_versions"]
        ...
        # A fresh forecast invalidates any in-flight What-If scenario results, since
        # they were scored against the previous scoring_data/forecast_raw.
    ...
    with st.spinner(gettext("Generating explanation...")):
        include_llm = is_llm_commentary_available() and st.session_state.get(
            "show_llm_commentary", False
        )
        narratives, explanations = build_series_narratives(
